...
class WETHeader(collections.namedtuple('WETHeader', ['url', 'length'])):
  ...
  def read(cls, f):
    ...
    if not line:
      # EOF
      return None
    while not line.startswith(cls.LENGTH_HEADER.encode("utf8")):
      if line.startswith(cls.URI_HEADER.encode("utf8")):
        url = line[len(cls.URI_HEADER.encode("utf8")):].strip()
      line = f.readline()
    ...
    # Read content
    length = int(line.decode("utf8").split(':')[1])
...
def wet_download_urls(wet_paths_url, tmp_dir, rm_after=True):
  ...
  with gzip.open(paths_gz) as f:
    path = f.readline()
    while path:
      download_path = S3_HTTP_PREFIX + path.decode("utf8")[:-1]
      yield download_path
      path = f.readline()
  if rm_after:
